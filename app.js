(() => {
  ...
  function loadInfo(){ campInfoEl.value = data.info || ''; }
  saveInfoBtn.addEventListener('click', ()=>{ data.info = campInfoEl.value || ''; save(); alert('Información guardada.'); });
  ...
  function renderAdminSearch(){
    ...
    qsa('#adminResults .small-btn').forEach(b=>{
      b.addEventListener('click', ev=>{
        ...
        if(action==='add'){ member.points = Number(member.points||0) + 1; } // individual +1
        if(action==='sub'){ 
          // si restamos puntos a miembro de equipo, restar 1 a todos
          team.members.forEach(m=>{ m.points = (Number(m.points)||0) - 1; });
        }

        save(); renderAll(); renderAdminSearch();
      });
    });
  }
  ...
  function openEdit(mid){
    ...
    editTeam.innerHTML='';
    data.teams.forEach(t=>{ const o=document.createElement('option'); o.value=t.id; o.textContent=t.name; if(t.id===parent.id) o.selected=true; editTeam.appendChild(o); });
  ...
  editForm.addEventListener('submit', async e=>{
    e.preventDefault();
    const mid = editId.value; const name = editName.value.trim(); const age = editAge.value ? Number(editAge.value):''; const gender = editGender.value; const tId = editTeam.value;
    const file = editPhoto.files && editPhoto.files[0]; const photo = file?await fileToBase64(file):null;
  ...
  deleteBtn.addEventListener('click', ()=>{
    ...
    for(const t of data.teams){ const idx = (t.members||[]).findIndex(x=>x.id===mid); if(idx>=0){ t.members.splice(idx,1); save(); break; } }
  ...
  function renderAdminTeams(){
    ...
    qsa('#teamsAdminList .small-btn').forEach(b=>{
      b.addEventListener('click', ev=>{
        const idd = ev.currentTarget.dataset.id; const action = ev.currentTarget.dataset.action;
        if(action==='rename'){ const newName = prompt('Nuevo nombre del equipo:'); if(newName){ const t=data.teams.find(x=>x.id===idd); if(t){ t.name=newName.trim(); save(); populateTeamSelects(); renderAdminTeams(); renderAll(); } } }
        else if(action==='del'){ if(!confirm('¿Eliminar equipo? se perderán sus miembros')) return; data.teams = data.teams.filter(x=>x.id!==idd); save(); populateTeamSelects(); renderAdminTeams(); renderAll(); }
      });
    });
  }

  function renderAll(){ renderTeamsTable(); renderPodium(); renderTeamsTop3(); renderRanking(searchAll.value||''); renderMejores(); loadInfo(); }
  searchAll.addEventListener('input', ()=> renderRanking(searchAll.value||''));

  load(); populateTeamSelects(); renderAll(); showSection('home');
